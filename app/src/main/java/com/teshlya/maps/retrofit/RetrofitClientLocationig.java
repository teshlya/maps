...
public class RetrofitClientLocationig extends RetrofitClient{

    public static RetrofitClientLocationig getInstance() {
        return new RetrofitClientLocationig();
    }
    ...
    private Call<ResultAddress> getCall(QueryParameters parameters) {
        return getRetrofitClient(Constants.BASE_URL_LOCATIONIQ)
                .getAddressFromLocationiq(parameters.getKey(),
                        parameters.getLat(),
                        parameters.getLon(),
                        "json");
    }
